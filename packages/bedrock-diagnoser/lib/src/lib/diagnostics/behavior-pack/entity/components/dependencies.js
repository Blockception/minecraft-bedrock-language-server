...
exports.behaviorpack_entity_components_dependencies = behaviorpack_entity_components_dependencies;
exports.checkMovements = checkMovements;
...
function behaviorpack_entity_components_dependencies(entity, context, diagnoser) {
    const components = context.components;
    checkMovements(diagnoser, components, entity);
    (0, components_1.components_dependencies)("entity", context, diagnoser, component_dependents_all, component_dependents_any);
}
function checkMovements(diagnoser, components, entity) {
    ...
    if (runtimeId === "minecraft:dolphin" || id === "minecraft:dolphin") {
        if (movementComps.length >= 1)
            diagnoser.add("runtime_identifier", `Entity runtime 'minecraft:dolphin' is not compatible with ${movementComps[0]}`, types_1.DiagnosticSeverity.error, "behaviorpack.entity.incompatible_runtime");
        else
            return;
    }
    // Check for glide
    ...
    if (movementComps.length === 0) {
        diagnoser.add("minecraft:movement", `Missing a movement component such as: 'minecraft:movement.basic'`, types_1.DiagnosticSeverity.error, "behaviorpack.entity.component.missing");
    }
    if (navigationComps.length === 0) {
        diagnoser.add("minecraft:movement", `Missing a movement component such as: 'minecraft:navigation.generic'`, types_1.DiagnosticSeverity.error, "behaviorpack.entity.component.missing");
    }
}
