...
class InMemoryContext {
    ...
    constructor(files) {
        this.files = files;
        this._data = new bc_minecraft_bedrock_project_1.MinecraftData(new bc_minecraft_bedrock_project_1.ProjectData(this));
    }
    getDocument(uri) {
    ...
    getFiles(_folder, _patterns, _ignores) {
    ...
    getDiagnoser(doc, project) {
        const d = new diagnoser_1.TestDocumentDiagnoser(doc, this, project);
...
// Helpers
// ---------------------------------------------------------------------------
function buildWorkspace(files = buildFileMap()) {
    ...
    const diagnoser = new src_1.Diagnoser(context);
    // Register packs
...
describe('Integration – multi-pack workspace diagnostic pipeline', () => {
    ...
    describe('no internal exceptions', () => {
        it('no debugger.internal.exception diagnostic for any file', () => {
            ...
            const internalErrors = context.diagnosers.flatMap((d) => d.items.filter((item) => item.code === 'debugger.internal.exception'));
