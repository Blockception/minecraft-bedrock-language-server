...
describe("Molang Pack Type Validation", () => {
    ...
    for (const test of testCases) {
        it(test.name, () => {
            const doc = bc_minecraft_bedrock_project_1.TestTextDocument.create(test.documentUri, "");
            const diagnoser = new diagnoser_1.TestDiagnoser();
            (0, molang_1.diagnose_molang_syntax_document)(doc, diagnoser, test.data);
            if (test.shouldHaveError) {
                ...
                const errorsAndWarnings = diagnoser.items.filter(item => item.severity >= 2 // warning or error
                );
...
//# sourceMappingURL=pack-type-validation.test.js.map