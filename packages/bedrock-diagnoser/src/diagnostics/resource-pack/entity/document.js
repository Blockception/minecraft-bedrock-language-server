...
function diagnose_entity_document(diag) {
    ...
    if (!entityGathered.molang) {
        entityGathered.molang = (0, molang_1.harvestMolang)(diagnoser.document.getText(), entity);
    ...
    bc_minecraft_bedrock_shared_1.Definition.forEach(anim_data.animations, (ref, anim_id) => (0, anim_or_controller_1.animation_or_controller_diagnose_implementation)(anim_id, entityGathered, diagnoser, description.particle_effects, description.sound_effects));
    bc_minecraft_bedrock_shared_1.Definition.forEach(anim_data.animationControllers, (ref, anim_id) => (0, anim_or_controller_1.animation_or_controller_diagnose_implementation)(anim_id, entityGathered, diagnoser, description.particle_effects, description.sound_effects));
    ...
    description.animation_controllers
        ...
        .forEach((id) => (0, diagnostics_1.diagnose_animation_controller_implementation)(id, entityGathered, diagnoser, {}));
    //Check render controllers
    description.render_controllers
        ?.map((controller) => getKey(controller))
        .filter((key) => key !== undefined)
        .forEach((key) => (0, diagnostics_2.render_controller_diagnose_implementation)(key, entityGathered, diagnoser));
    //Check models
