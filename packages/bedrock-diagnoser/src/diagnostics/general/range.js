...
function general_range_integer_diagnose(value, diagnoser, range) {
    ...
    if (index >= 0) {
        ...
        if (lowerText !== '' &&
            !(0, integer_1.general_integer_diagnose)(bc_minecraft_bedrock_shared_1.OffsetWord.create(lowerText, value.offset), diagnoser, range))
            co = false;
        if (upperText !== '' &&
            !(0, integer_1.general_integer_diagnose)(bc_minecraft_bedrock_shared_1.OffsetWord.create(upperText, value.offset + index + 2), diagnoser, range))
            co = false;
...
function general_range_float_diagnose(value, diagnoser, range) {
    ...
    if (index >= 0) {
        ...
        let co = true;
        //If the text is filled, diagnose it as integer. if that returns false it has an error, then mark 'co'ntinue as false
        if (lowerText !== '' && !(0, float_1.general_float_diagnose)(bc_minecraft_bedrock_shared_1.OffsetWord.create(lowerText, value.offset), diagnoser, range))
            co = false;
        if (upperText !== '' &&
            !(0, float_1.general_float_diagnose)(bc_minecraft_bedrock_shared_1.OffsetWord.create(upperText, value.offset + index + 2), diagnoser, range))
            co = false;
        //Return if marked to not continue
        if (!co) {
            return true;
