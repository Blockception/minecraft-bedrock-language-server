...
const component_test = {
    ...
    'minecraft:destructible_by_mining': (name, component, context, diagnoser) => {
        ...
        if (!destroyTime)
            return;
        component.item_specific_speeds?.forEach((specific_speed) => {
    ...
    'minecraft:placement_filter': (name, component, context, diagnoser) => {
        for (const condition of component.conditions) {
            condition.block_filter
                ?.map((block) => typeof block === 'string' ? block : 'name' in block ? block.name : undefined)
                .forEach((block) => {
    ...
    'minecraft:material_instances': (name, component, context, diagnoser) => {
        ...
        (0, bc_minecraft_bedrock_shared_1.safeObjectKeys)(component)
        ...
        if (new Set((0, bc_minecraft_bedrock_shared_1.safeObjectKeys)(component)
            .filter((value) => typeof component[value] === 'object' && component[value] !== null)
            .map((value) => component[value].render_method)).size > 1)
            diagnoser.add(name, 'Custom blocks were never intended to support multiple different render_method inside this component', types_1.DiagnosticSeverity.warning, 'behaviorpack.block.components.multiple_render_methods');
    },
