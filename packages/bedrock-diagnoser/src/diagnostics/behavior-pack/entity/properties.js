...
function diagnose_entity_enum_property_definition(property, diagnoser) {
    ...
    if (def !== undefined) {
        if (property.values?.indexOf(def) === -1 && !(0, bc_minecraft_molang_1.isMolang)(def)) {
    ...
    if (property.values !== undefined) {
        ...
        if (property.values.length > 16) {
            diagnoser.add(`properties/${name}`, `Entity has too many values: ${property.values?.length}, expected 16 or less`, types_1.DiagnosticSeverity.error, 'behaviorpack.entity.property.enum.values.count');
        }
...
function check_entity_property_usage(definition, name, value, parent, diagnoser) {
    switch (definition.type) {
        ...
        case 'enum':
            ...
            if (definition.values?.indexOf(value) === -1 && !(0, bc_minecraft_molang_1.isMolang)(value)) {
                diagnoser.add(`${parent}/${name}/${value}`, `Property value is not in the list of enum values: ${value}, expecting ${definition.values?.join(', ')}`, types_1.DiagnosticSeverity.error, 'behaviorpack.entity.property.enum.value');
            }
            break;
    }
}
