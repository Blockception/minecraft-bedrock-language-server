...
function diagnose_mcfunction_commands(command, diagnoser, edu) {
    ...
    if (info.length === 0) {
        ...
        if (command.subType === bc_minecraft_bedrock_command_1.ParameterType.executeSubcommand && bc_minecraft_bedrock_command_1.CommandData.ExecuteSubcommands[keyCommand] !== undefined) {
            return diagnoser.add(command.parameters[0].offset, `Unknown syntax for: "${keyCommand}"`, types_1.DiagnosticSeverity.error, `minecraft.commands.${keyCommand}.syntax`);
        }
        return (0, minecraft_1.minecraft_check_command)(command.parameters[0], diagnoser, edu);
    }
    const data = info[0];
    ...
    (0, minecraft_1.minecraft_coordinate_set_diagnose)(info, command.parameters, diagnoser);
}
/**Switch data*/
const ParameterDiagnostics = {
    ...
    [bc_minecraft_bedrock_command_1.ParameterType.scoreComparator]: diagnose_1.mode_score_comparator_diagnose,
