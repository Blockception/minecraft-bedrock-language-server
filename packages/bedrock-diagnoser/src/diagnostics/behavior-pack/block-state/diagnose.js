        ...
        for (let i = 0; i < split.length; i++) {
            ...
            if (state.length == 2) {
                ...
                if (property.startsWith('"') && property.endsWith('"')) {
                    ...
                    diagnoser.add(blockId, `Invalid state: '${property}' in '${item}' on the block state definition: '${states.text}', needs to be a string literal with ""`, types_1.DiagnosticSeverity.error, 'behaviorpack.block.states.invalid');
                }
            }
            else if (state[0] !== '') {
                diagnoser.add(states, `Invalid state: '${item}' in the block command, needs to be in the format ["state"=value] :`, types_1.DiagnosticSeverity.error, 'behaviorpack.block.states.invalid');
            }
        }
    }
    else {
        diagnoser.add(states, `Invalid states: '${states.text}' in the block command, needs to be a list with []`, types_1.DiagnosticSeverity.error, 'behaviorpack.block.states.invalid');
    }
...
function check_block_definition(blockDefinition, location, diagnoser) {
    ...
    if (block.states.length == 0 && blockDefinition.states.length > 0) {
        ...
        diagnoser.add(location, `Block: ${block.id} has no defined states`, types_1.DiagnosticSeverity.error, 'behaviorpack.block.states.missing');
        return;
    }
        ...
        if (stateData.name === state.property) {
            ...
            if (stateData.type === 'string') {
                if (actual.startsWith('"') && actual.endsWith('"')) {
                    ...
                    diagnoser.add(location, `Invalid state value: '${state.value}' for state: '${state.property}' in the block definition: '${data.id}', needs to be a string literal with ""`, types_1.DiagnosticSeverity.error, 'behaviorpack.block.states.invalid');
                    return;
                }
            }
            ...
            diagnoser.add(location, `Invalid state value: '${state.value}' for state: '${state.property}' in the block definition: '${data.id}'\nValid values are: ${values.join(',')}`, types_1.DiagnosticSeverity.error, 'behaviorpack.block.states.invalid');
            return;
        }
    }
    //No state matching found
    diagnoser.add(location, `Missing state: '${state.property}' in the block definition: '${data.id}'`, types_1.DiagnosticSeverity.error, 'behaviorpack.block.states.missing');
}
