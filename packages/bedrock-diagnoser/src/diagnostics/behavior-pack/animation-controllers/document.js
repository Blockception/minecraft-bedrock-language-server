...
function diagnose_animation_controller_document(diagnoser) {
    ...
    Object.entries(controllers.animation_controllers).forEach(([id, controller]) => {
        ...
        (0, duplicate_check_1.no_other_duplicates)('behaviorpack.animationControllers', diagnoser.context.getProjectData().projectData.behaviorPacks.animationControllers, id, diagnoser);
        Object.values(controller.states).forEach((state) => {
            state.on_entry?.forEach((item) => (0, commands_1.json_commandsCheck)(item, diagnoser));
            state.on_exit?.forEach((item) => (0, commands_1.json_commandsCheck)(item, diagnoser));
        });
    });
}
