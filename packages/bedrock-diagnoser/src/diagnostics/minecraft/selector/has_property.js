...
function entity_has_property(attr, diagnoser) {
    ...
    const key = bc_minecraft_bedrock_shared_1.OffsetWord.create(attr.key, attr.offset);
    ...
    if (key.text === 'property') {
        ...
        if (!hasAnyEntity) {
            diagnoser.add(value, `No entity has the property '${value.text}'`, types_1.DiagnosticSeverity.error, 'minecraft.selector.has_property.notfound');
            return false;
        }
        return true;
    }
    ...
    entities = entities.filter((entity) => entity.properties.some((item) => {
        if (item.name !== key.text) {
            return false;
        }
        switch (item.type) {
            case 'bool':
