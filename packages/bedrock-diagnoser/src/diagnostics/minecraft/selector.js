...
function minecraft_selector_diagnose_hard(value, diagnoser, pattern) {
    ...
    if (!bc_minecraft_bedrock_types_1.Minecraft.Selector.Selector.isValidType(selector)) {
        result = false;
        diagnoser.add(value, `Unknown selector type: ${selector.selectorType}`, types_1.DiagnosticSeverity.error, 'minecraft.selector.type.invalid');
    }
    //Check attributes
    ...
    for (const name of names) {
        ...
        if (attributes) {
            result &&= minecraft_selector_attribute_diagnose_hard(name, attributes, selector, diagnoser);
        }
    }
    return result;
}
