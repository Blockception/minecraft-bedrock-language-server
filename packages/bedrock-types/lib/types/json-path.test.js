...
describe("Json", () => {
    it("Json Path - Resolve", () => {
        const offset = json_path_1.JsonPath.resolve(jsonData, "minecraft:entity/description/identifier");
        expect(offset).toEqual(jsonData.indexOf("identifier"));
    });
    it("Json Path - Resolve2", () => {
        const offset = json_path_1.JsonPath.resolve(jsonData, "example:foo");
        expect(offset).toEqual(jsonData.indexOf("example:foo"));
    });
    it("Json Path - Resolve3", () => {
        const offset = json_path_1.JsonPath.resolve(jsonData, "minecraft:damage_sensor");
        expect(offset).toEqual(jsonData.indexOf("minecraft:damage_sensor"));
    });
    it("Json Path - Resolve4", () => {
        const offset = json_path_1.JsonPath.resolve(jsonData, "minecraft:entity/components/minecraft:damage_sensor");
        expect(offset).toEqual(jsonData.indexOf("minecraft:damage_sensor"));
    });
    it("Json Path - Resolve Wrapper", () => {
        const offset = json_path_1.JsonPath.resolve(jsonWrapper, "minecraft:entity/description/identifier");
        expect(offset).toEqual(jsonData.indexOf("identifier"));
    });
});
