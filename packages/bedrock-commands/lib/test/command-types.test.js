...
describe("Command", () => {
    it("parse 1 - simple test", () => {
        const comm = main_1.Command.parse("scoreboard players set @a[scores={foo=1..}] foo 1", 0);
    ...
    it("parse 2 - condensed ~ coordinates", () => {
        const comm = main_1.Command.parse("tp @s ~~~", 0);
    ...
    it("parse 4 - unknown command", () => {
        const comm = main_1.Command.parse("fakecommand fakeparameter @s a", 0);
        expect(comm.parameters).toHaveLength(4);
        expect(comm.getCommandData(false)).toHaveLength(0);
    });
    it("parse 5 - offset", () => {
        const comm = main_1.Command.parse("    execute @s ~~~", 0);
        expect(comm.parameters).toHaveLength(5);
        expect(comm.parameters[0].offset).toEqual(4);
    });
    it("subcommand 1", () => {
        const comm = main_1.Command.parse("execute @s[scores={foo=1..}] ~ ~ ~ tp @a[tag=target] @s", 0);
    ...
    it("subcommand 2", () => {
        const comm = main_1.Command.parse("execute @s[scores={foo=1..}] ~ ~ ~ detect ~ ~ ~ minecraft:air -1 tp @a[tag=target] @s", 0);
        const sub = comm.getSubCommand();
        if (sub) {
            expect(sub.parameters).toHaveLength(3);
            expect(sub.parameters[0].text).toEqual("tp");
    ...
    it("cursorIndex must be in the first word", () => {
        const comm = main_1.Command.parse("execute @s[scores={foo=1..}]", 0);
        expect(comm.findCursorIndex(29)).toEqual(2);
    });
    it("cursorIndex must be in the first word if before", () => {
        const comm = main_1.Command.parse("execute @s[scores={foo=1..}]", 30);
        expect(comm.findCursorIndex(5)).toEqual(0);
    });
});
//# sourceMappingURL=command-types.test.js.map