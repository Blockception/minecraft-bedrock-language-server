...
function CheckObjective(Com, doc) {
    ...
    if (objectiveMode.text === 'add') {
        ...
        return general_info_1.GeneralInfo.create(ID.text, bc_minecraft_bedrock_shared_1.Location.create(doc.uri, offset), types_1.Documentation.getDoc(doc, () => {
...
function CheckPlayer(Com, doc) {
    if (Com.parameters.length > 3) {
        ...
        if (!Selector.text.startsWith('@') && types_2.Text.UnQuote(Selector.text) !== '*') {
            return general_info_1.GeneralInfo.create(Selector.text, bc_minecraft_bedrock_shared_1.Location.create(doc.uri, Selector.offset), types_1.Documentation.getDoc(doc, () => `The fake player: ${Selector.text}`, Selector.offset));
        }
    }
    return undefined;
}
