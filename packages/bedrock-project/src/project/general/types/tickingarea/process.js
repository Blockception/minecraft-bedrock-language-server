...
function process(Command, doc) {
    //tickingarea add
    if (Command.parameters[1]?.text !== 'add')
        return;
...
function ProcessCircleCommand(Command, doc) {
    ...
    return general_info_1.GeneralInfo.create(Name, bc_minecraft_bedrock_shared_1.Location.create(doc.uri, parameters[3].offset), types_1.Documentation.getDoc(doc, () => {
