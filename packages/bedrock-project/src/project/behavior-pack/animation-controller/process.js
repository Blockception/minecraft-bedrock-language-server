...
function process(doc) {
    ...
    return Object.entries(imp.animation_controllers)
        ...
        Object.values(controller.states).forEach((state) => {
            bc_minecraft_bedrock_shared_1.Conditional.forEach(state.animations, (reference) => item.animations.using.add(reference));
            references_1.Using.add(item.events, state.on_entry?.filter((entry) => entry.startsWith('@s ')).map((entry) => entry.slice(3)));
            references_1.Using.add(item.events, state.on_exit?.filter((entry) => entry.startsWith('@s ')).map((entry) => entry.slice(3)));
        });
        return item;
    });
}
