...
exports.LocalizationIDs = void 0;
...
function activate(context) {
    async function FillIdByName() {
        ...
        return command.process('resource_pack/texts/en_US.lang', bc_minecraft_bedrock_vanilla_data_1.Vanilla.GithubFiles.source);
    }
    context.subscriptions.push(vscode_1.commands.registerCommand(ide_shared_1.Commands.FillIdByName, FillIdByName));
}
...
class FillIdByNameCommand {
    ...
    async download(uri, filepath) {
        ...
        return vscode_1.window.withProgress(progressOptions, async (progress) => {
            ...
            await fetch(uri, options)
                ...
                .catch((err) => {
                vscode_1.window.showErrorMessage('Failed to download vanilla file\n', uri + '\n', filepath + '\n', JSON.stringify(err));
            })
                .finally(() => {
    ...
    async process(key, source) {
        ...
        let contents;
        try {
            contents = new TextDecoder('utf-8').decode(await vscode_1.workspace.fs.readFile(vscode_1.Uri.file(filepath)));
        }
        catch (err) {
            vscode_1.window.showErrorMessage('Failed to read vanilla file', filepath, JSON.stringify(err));
            return;
        }
        // Parse contents
    ...
    quickPickWithPrefill(items, prefill) {
...
function getIdByLocalizationID(key) {
    const id = Object.keys(exports.LocalizationIDs).find((k) => exports.LocalizationIDs[k] === key);
    return 'minecraft:' + (id ? id : key.replace('tile.', '').replace('item.', '').replace('.name', '').trim());
}
//# sourceMappingURL=fill-id-by-name.js.map