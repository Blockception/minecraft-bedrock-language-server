...
function provideReferences(context) {
    const { document, position } = context;
    //Gets start of line
    const startP = vscode_languageserver_1.Position.create(position.line, 0);
    const line = document.getLine(startP.line);
    const lineOffset = document.offsetAt(startP);
    return Command.provideReferences(context, new bc_vscode_words_1.OffsetWord(line, lineOffset));
}
